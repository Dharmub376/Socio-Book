...
function Post() {
    ...
    useEffect(() => {
        ...
        const initializedPosts = savedPosts.map((post: PostType) => ({
            ...post,
            likes: post.likes || {},
    ...
    const toggleMenu = (postId: string, e: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
        e.stopPropagation();
        setMenuOpen(menuOpen === postId ? null : postId);
    };

    interface HasUserLikedPost {
        likes?: { [userId: string]: boolean };
    }

    const hasUserLiked = (post: HasUserLikedPost): boolean => {
        return !!(post.likes && post.likes[user.id]);
    };

    interface HasUserBookmarkedPost {
        bookmarks?: { [userId: string]: boolean };
    }

    const hasUserBookmarked = (post: HasUserBookmarkedPost): boolean => {
    ...
    interface PostTypeWithAllFields extends PostType {
        likes?: LikesType;
    ...
    const countLikes = (post: PostTypeWithAllFields): number => {
